import React from 'react';

const Partners = () => {
    return (

        <section className="brand-dark-grey">
            <div className="container ptb-6">
                <div className="row">
                    <div className="col s12 m12 l10 offset-l1 center-align">
                        <h2 className="white-text mt0">Future Fownders Partners</h2>
                        <p className="white-text center">Our Corporate, Community and School partnerships are vital to the continuation and ongoing impact of the Future Fownders Program. We humbly thank the following partners: </p>
                        <h5 className="white-text normal">Corporate & Community Beneficiaries:</h5>
                        <br />
                    </div>
                </div>
                <div className="row mb0">
                    <div className="col s6 m4 center-align">
                        <img className="responsive-img" alt="Brown and Caldwell" width={140} src="http://www.fulcrumapp.com/assets/img/cases/brownandcaldwell-logo.jpg" />
                    </div>
                    <div className="col s6 m4 center-align">
                        <img className="responsive-img" alt="The Daily Bread" width={90} src="https://yt3.ggpht.com/a-/AJLlDp2SrzAdU_tJKJwXhJ9XDmkLv859pzRFTSuv5g=s900-mo-c-c0xffffffff-rj-k-no" />
                    </div>
                    <div className="col s6 m4 center-align">
                        <img className="responsive-img" alt="Rutgers CUEED" width={260} src="https://farm1.staticflickr.com/981/41831827251_e3d6fdda3a_m.jpg" />
                    </div>
                </div>
                <div className="row mb0">
                    <div className="col s12 m12 l10 offset-l1 center-align">
                        <h5 className="white-text normal">Mentorship & Collaborative Experiences:</h5>
                        <br />
                    </div>
                </div>
                <div className="row">
                    <div className="col s6 m4 center-align">
                        <img className="responsive-img" alt="Devils" width={90} src="https://upload.wikimedia.org/wikipedia/en/thumb/9/9f/New_Jersey_Devils_logo.svg/1200px-New_Jersey_Devils_logo.svg.png" />
                    </div>
                    <div className="col s6 m4 center-align">
                        <img className="responsive-img" alt="TheProfitRoom" width={110} src="https://farm1.staticflickr.com/864/41788240332_0fb48f8fe8_m.jpg" />
                    </div>
                    <div className="col s6 m4 center-align">
                        <img className="responsive-img" alt="Fit Lifestyle Box" src="https://farm1.staticflickr.com/972/27829714468_e9c7c93b5a_m.jpg" />
                    </div>
                    <div className="row mb0">
                    <div className="col s12 m12 l10 offset-l1 center-align">
                        <h5 className="white-text normal">Future Fownders Participating Schools:</h5>
                        <br />
                    </div>
                </div>
                <div className="row">
                    <div className="col s6 m4 center-align">
                        <img className="responsive-img" alt="East Side Red Raiders" width={90} src="http://content.nps.k12.nj.us/wp-content/uploads/sites/58/2014/08/EAS.png" />
                    </div>
                    <div className="col s6 m4 center-align">
                        <img className="responsive-img" alt="People's Rep" width={90} src="data:image/png;base64," />
                    </div>
                    <div className="col s6 m4 center-align">
                        <br className="hide-on-small-and-down"/>
                        <img className="responsive-img" alt="Link Charter" width={230} src="https://farm1.staticflickr.com/867/27962641338_fb3e552b4f_m.jpg"/>
                    </div>
                    <div className="col s6 m4 center-align">
                        <img className="responsive-img" alt="Chancellor" width={100} src="http://static.wixstatic.com/media/6545f7_8d35132bc9df4ed7b39d5433ab5cfbfd.png/v1/fill/w_224,h_212,al_c,lg_1/6545f7_8d35132bc9df4ed7b39d5433ab5cfbfd.png" />
                    </div>
                    <div className="col s6 m4 center-align">
                        <br className="hide-on-small-and-down"/>
                        <img className="responsive-img" alt="Platinum Properties" width={230} src="https://fownders.s3.us-east-2.amazonaws.com/ff-partner-platinum-white.png"/>
                    </div>
                </div>
                <div className="row mb0">
                    
                    
                </div>
            </div>
            </div>
        
        </section>
    )
}

export default Partners;
